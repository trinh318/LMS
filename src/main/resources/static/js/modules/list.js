    ...
    deleteAllForm.addEventListener("submit", (event) => {
        ...
        fetch("/modules/delete-selected", {
            ...
            .then((response) => {
                ...
                if (redirectUrl && redirectUrl.includes("modules")) {
                ...
                if (!response.ok) {
                    return response.text().then((text) => {
                        throw new Error(text || "Failed to delete modules");
                    });
                }
            ...
            .catch((error) => {
                ...
                alert("An error occurred while deleting modules: " + error.message);
            });
    });
...
document
    .getElementById("deleteAllModulesForm")
    .addEventListener("submit", async (e) => {
        ...
        try {
            ...
            const response = await fetch("/modules/delete-all", {
                method: "POST",
                headers: headers,
            });
            ...
            if (!response.ok) {
                ...
                throw new Error(errorText || "Failed to delete all modules");
            }

            // Hide modal if it exists
            const modalEl = document.getElementById("deleteAllModulesModal");
            if (modalEl) {
                const modal = bootstrap.Modal.getInstance(modalEl);
                if (modal) modal.hide();
            }
            ...
            alert(
                "An error occurred while deleting all modules: " + error.message
            );
        }
    });